...
\sparagraph{Best Configuration.} Figure~\ref{best_config} shows the compression factor improvement against FSST when choosing the best configuration for each file independently (Best) from the 7 configurations shown in Figure~\ref{cf_combined} and including FSST itself as an $8^{th}$ configuration. The average compression factor improvement from FSST is at 8.8\%, which is not significantly higher than always compressing with BtrFSST, that shows a 7.3\% average per-file compression factor improvement. Below are the counts for each configuration, of how many files it had the best compression factor:

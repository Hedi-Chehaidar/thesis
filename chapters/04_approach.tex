...
Figure~\ref{dp_example} shows an example for compressing a text using the dp formula of Eq.~\eqref{formula2}. \texttt{dp[0]} tells that we needed 5 bytes to compress the text given the available symbols. \texttt{opt} contains one best symbol that yields to the best compression of the suffix beginning from the corresponding position. The values for both tables are filled backward (from right to left) simultaneously according to the used formula. The arrows show the steps to recover the symbols used for compression needed for counting frequencies in the \texttt{compressCount} function.
...
\sparagraph{Example without Pruning.} Figure~\ref{no_pruning} shows the symbol selection process without pruning. The previous symbol table used for counting had only three symbols: ``a'', ``b'', and ``c''. The frequencies in the table \texttt{count} of the used symbols for compression and their concatenations and the corresponding gains (\texttt{frequency} $\times$ \texttt{length}) are computed by \texttt{compressCount}. We did not include double concatentations (\texttt{count3}) for simplicity.

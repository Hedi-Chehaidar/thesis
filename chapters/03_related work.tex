...
\sparagraph{Limitation.} Although the original FSST algorithm is highly optimized for runtime, particularly when using the SIMD variant, the greedy strategy employed by the \texttt{findLongestSymbol} function does not always select the optimal symbol, both during compression and during symbol table construction. This leaves room for improving the compression factor by adopting a different optimization strategy that more carefully considers conflicts between overlapping symbols.

\subsection{FSST+}

FSST+~\cite{fsstplus} is a recent extension of the FSST algorithm, where common prefixes from consecutive rows are extracted and compressed independantly from the rest of the strings. 

Rows are grouped in fixed-size blocks (usually of 128 strings), where they are next sorted lexicographically to obtain longer shared prefixes. Dynamic programming is afterwards used to choose the optimal partitioning of the strings of each block into similarity chuncks, where a single prefix is determined for each chunck. Each row of the chunck is assigned a jump-back offset pointing to the used prefix, while also storing the prefix length. 

FSST+ exploits prefix redundancies and achieves significantly higher compression factors than FSST, especially in prefix-rich data, while keeping a linear runtime to the corpus size.  

Extracted prefixes and the rest of the strings are compressed using the original FSST. Therefore, the contributions of this thesis can optimize further the compression factors, as they improve the FSST algorithm itself. 

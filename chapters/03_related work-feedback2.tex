...
% Mihail: with *an*?
The LZ77 general technique works by scanning a block with a usual size of 128 KB as input, detecting byte sequences that appeared before. Repeated sequences are encoded as a pair (\texttt{offset}, \texttt{length}) where \texttt{offset} references the first appearance of the sequence in the output stream.
...
LZ4 decompression is very similar to just performing memory copy ('memcpy') operations, making it bounded by memory bandwith with speed of multiple GB/s per core, typically reaching RAM speed limits on multi-core systems.
...
% Mihail: Here note the quotes ``''. It can be seen in the PDF if it's written or copied :).
...
Figure~\ref{fsst_example} shows the first 10 symbols of an example symbol table, with an additional array for symbol lengths. For example, the first string is compressed into three bytes (codes), where each code is the index of the corresponding symbol in the symbol table. 
...
The function used to find the longest matching symbol is called \texttt{findLongestSymbol}, and its implementation is shown in Figure~\ref{algorithm3}. The same function is also used during symbol table construction.
...
\centering
  ...
  \caption{FSST symbol table construction \cite{fsst}.}
  \label{algorithm3}
\end{figure}

...
% Mihail: You sometimes put the comments above the line or right to the line.
% Please be consistent.
% You can also merge `start, end = self.sIndex[data[i]], self.sIndex[data[i] + 1]'.
% I usually like some spaces in the variables, like: [0] * (n + 1) or `dp[i + 1]'. It gives the reader some space.
...
% Mihail: Please make sure to not have code which goes through pages. I think there's the `[t]' option in latex. Or ask chat.
...
Having the trie as the structure used for the \texttt{findLongestSymbol} function, there is no need for the \texttt{sIndex} table and therefore the \texttt{makeIndex} function. As a result, including this method in the original C++ implementation of FSST~\cite{fsst_repo}, that already does not use \texttt{makeIndex}, becomes easier.     
...
% Mihail: Also here -- seems to span over 2 pages.
% Mihail: Code looks good, but I'd do some spcaces for pack3(a, b, c)!
...
% Mihail: Code doesn't look intended!!!

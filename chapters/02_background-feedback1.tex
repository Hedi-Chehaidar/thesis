...
% Mihail: I don't think we really need this.
...
% Mihail: We really don't need this, since it's not related to our work.
% Mihail: The above paragraph is more than enough!
...
\subsection{Max Heap}
% Mihail: Note: This hyphens are usually from ChatGPT. Please use a simple one: `-'.
...
% Mihail: Please be consistent: `max heap' vs. `max-heap'.
...
% Mihail: Please define the data you'll be using in the sequel. Say sth like:
% Mihail: In the sequel, consider the following data {(42, fs), ...}.
% Mihail: Also mention *why* you're choosing pairs. Probably this is the kind of data you'll be using later in the algorithm itself, right?

% Mihail: Usually, when having a figure, we also need to explain in the _text_ what the figure is about. Now it's just some figures withhout any explanaation. This not recommended in scientific writing.
...
\end{tikzpicture}
\caption{Initial max heap ordered by frequency.}
\label{fig:maxheap-initial}
\end{figure}
...
\end{tikzpicture}
\caption{Max heap after extracting the maximum element ⟨42, fs⟩.}
\label{fig:maxheap-pop}
\end{figure}
...
\noindent
\newline
% Mihai: *The* following trie.
Following trie stores the 5 strings "fs", "fst", "db",

% Mihail: Missing Oxford comma here, after `dbms'.
"dbms" and "zip" with a mapping for each string as an integer stored in the corresponding terminal node.

% Mihail: Also please explain the figure about the trie. Say sth like: For instance, the string `dbms' terminates at ..., while having `db' as ...
% Mihail: Also motivated why you're building the trie: "We'll be using it later to index the symbol table", I suppose. This will help the reader pay attention to it, such that they know why you're introducing this data structure.
...
\end{tikzpicture}
\caption{Example trie storing short strings. Terminal nodes are annotated with integer metadata.}
